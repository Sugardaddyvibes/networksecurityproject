import os
import sys
import json
from dotenv import load_dotenv
import pymongo.mongo_client

load_dotenv()
MONGO_DB_URL= os.getenv("MONGO_DB_URL")
print(MONGO_DB_URL)

import certifi
## certficate autority
ca=certifi.where()
import pandas as pd
import numpy as np
import pymongo 
from networksecurity.exception.exception  import NetworkSecurityExecption
from networksecurity.logging.logger import logger
class NetworkDataExtract():
    def __init__(self):
        try:
            pass
        except Exception as e:
            raise NetworkSecurityExecption(e,sys)
    def cv_to_json(self,file_path):
        try:
            data=pd.read_csv(file_path)
            data.reset_index(drop=True,inplace=True)
            records=list(json.loads((data.T.to_json())).values())
            return records
        except Exception as e:
            raise NetworkSecurityExecption(e,sys)
    def insert_data_into_monogo_db(self,records,database,collection):
        try:
            self.database=database
            self.records=records
            self.collection=collection
            self.mongo_client=pymongo.MongoClient(MONGO_DB_URL)
            database=self.mongo_client[database]
            collection=database[collection]
            collection.insert_many(records)
            return(len(records))
        except Exception as e:
            raise NetworkSecurityExecption(e,sys)

if __name__ == "__main__":
    FILE_PATH =r"Network_Data\phisingData.csv"
    DATABASE = "yeritheplug"
    COLLECTION = "NetworkData"

    networkonj = NetworkDataExtract()  

    # CSV to JSON conversion
    records = networkonj.cv_to_json(file_path=FILE_PATH)
    print(records)
    no_of_records = networkonj.insert_data_into_monogo_db(records,DATABASE,COLLECTION)
    print(f"Successfully inserted {no_of_records} records.")
    
    
